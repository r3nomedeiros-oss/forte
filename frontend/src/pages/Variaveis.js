...
function Variaveis() {
  ...
  const moverItem = async (tipo, indexAtual, direcao) => {
    ...
    setMovendo(`${lista[indexAtual].id}-${direcao}`);
    
    // Criar nova lista com posições trocadas
    const novaLista = [...lista];
    const temp = novaLista[indexAtual];
    novaLista[indexAtual] = novaLista[novoIndex];
  ...
  // Componente para renderizar lista
          ...
          {lista.map((v, index) => (
            ...
            >
              ...
              <div style={{display: 'flex', flexDirection: 'column', gap: '2px'}}>
                <button
                  type="button"
                  onClick={() => moverItem(tipo, index, -1)}
                  disabled={index === 0 || movendo !== null}
                  style={{
                    width: '28px',
                    height: '24px',
                    padding: '0',
                ...
                <button
                  type="button"
                  onClick={() => moverItem(tipo, index, 1)}
                  disabled={index === lista.length - 1 || movendo !== null}
                  style={{
                    width: '28px',
                    height: '24px',
                    padding: '0',
